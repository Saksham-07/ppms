...
Future<List<ProductionData>> fetchProdData(String from, String to, String units, String vgUnit) async {
  print('wsjdncw');
  final response = await http.get(
    Uri.parse('${TBaseURL.baseUrl}mngmnt_review_vg?type=ProductionDtl&unitVg=$vgUnit&fromDate=$from&toDate=$to&unit=$units&proc_type=MngmntReviewProductionDtl'),
  );

  print('${TBaseURL.baseUrl}mngmnt_review_vg?type=ProductionDtl&unitVg=$vgUnit&fromDate=$from&toDate=$to&unit=$units&proc_type=MngmntReviewProductionDtl');
...
class ProdDataSourceR extends DataGridSource {
  ProdDataSourceR({required List<ProductionData> prodData}) {
...
// Main widget
class ProductionDataRightTable extends StatefulWidget {
  final String from;
  final String to;
  final String units;
  final String vgUnit;

  const ProductionDataRightTable({
    super.key,
    required this.from,
    required this.to,
    required this.units,
  ...
  @override
  State<ProductionDataRightTable> createState() => _ProductionDataRightTableState();
}

class _ProductionDataRightTableState extends State<ProductionDataRightTable> {
  late Future<List<ProductionData>> prodData;

  @override
  void initState() {
    super.initState();
    prodData = fetchProdData(widget.from, widget.to, widget.units, widget.vgUnit);
  }
        ...
        return Container(
          ...
          child: SizedBox(
            ...
            child: SfDataGrid(
              ...
              source: ProdDataSourceR(prodData: snapshot.data!),
              stackedHeaderRows: [
