...
Future<List<AttManpowerData>> fetchAttManpower(String from, String to, String units) async {
  final response = await http.get(
    Uri.parse(
        'http://14.142.248.34:10008/management?proce_type=attManpower&from=&to=&units=$units'),
  );
...
class AttManpowerDataSource extends DataGridSource {
  AttManpowerDataSource({required List<AttManpowerData> attManpoerData}) {
    _dataGridRows = attManpoerData.map<DataGridRow>((data) {
      return DataGridRow(cells: [
        ...
        DataGridCell<int?>(columnName: 'MaleOnRoll', value: (data.maleStaffOnrollToday)),
        DataGridCell<int?>(columnName: 'MaleJoined', value: (data.maleStaffJoinedToday)),
        DataGridCell<int?>(columnName: 'MaleLeft', value: (data.maleStaffLeftToday)),
        DataGridCell<double?>(columnName: 'MaleAttrition', value: double.parse((data.maleStaffTotalAttr).toStringAsFixed(2))),
        DataGridCell<int?>(columnName: 'FemaleOnRoll', value: (data.femaleStaffOnrollToday)),
        DataGridCell<int?>(columnName: 'FemaleJoined', value: (data.femaleStaffJoinedToday)),
        DataGridCell<int?>(columnName: 'FemaleLeft', value: (data.femaleStaffLeftToday)),
        DataGridCell<double>(columnName: 'FemaleAttrition', value: double.parse(data.femaleStaffTotalAttr.toStringAsFixed(2))),
        DataGridCell<int?>(columnName: 'TotalOnRoll', value: (data.staffOnrollToday)),
        DataGridCell<int?>(columnName: 'TotalJoined', value: (data.staffJoinedToday)),
        DataGridCell<int?>(columnName: 'TotalLeft', value: (data.staffLeftToday)),
        DataGridCell<double?>(columnName: 'TotalAttrition', value: double.parse((data.staffTotalAttr).toStringAsFixed(2))),
      ]);
    }).toList();
    ...
    if (attManpoerData.isNotEmpty) {
      ...
      _dataGridRows.add(DataGridRow(cells: [
        ...
        DataGridCell<int>(columnName: 'MaleOnRoll', value: totals['maleStaffOnrollToday']),
        DataGridCell<int>(columnName: 'MaleJoined', value: totals['maleStaffJoinedToday']),
        DataGridCell<int>(columnName: 'MaleLeft', value: totals['maleStaffLeftToday']),
        DataGridCell<double>(columnName: 'MaleAttrition', value: totals['maleStaffTotalAttr']),
        DataGridCell<int>(columnName: 'FemaleOnRoll', value: totals['femaleStaffOnrollToday']),
        DataGridCell<int>(columnName: 'FemaleJoined', value: totals['femaleStaffJoinedToday']),
        DataGridCell<int>(columnName: 'FemaleLeft', value: totals['femaleStaffLeftToday']),
        DataGridCell<double>(columnName: 'FemaleAttrition', value: totals['femaleStaffTotalAttr']),
        DataGridCell<int>(columnName: 'TotalOnRoll', value: totals['staffOnrollToday']),
        DataGridCell<int>(columnName: 'TotalJoined', value: totals['staffJoinedToday']),
        DataGridCell<int>(columnName: 'TotalLeft', value: totals['staffLeftToday']),
        DataGridCell<double>(columnName: 'TotalAttrition', value: totals['staffTotalAttr']),
      ]));
    }
  }
  ...
  Map<String, dynamic> _calculateTotals(List<AttManpowerData> data) {
    ...
    return {
      'maleStaffOnrollToday': data.fold(0, (sum, item) => sum + item.maleStaffOnrollToday),
      'maleStaffJoinedToday': data.fold(0, (sum, item) => sum + item.maleStaffJoinedToday),
      'maleStaffLeftToday': data.fold(0, (sum, item) => sum + item.maleStaffLeftToday),
      'maleStaffTotalAttr': calculateAverage(data.map((item) => item.maleStaffTotalAttr).toList()),
      'femaleStaffOnrollToday': data.fold(0, (sum, item) => sum + item.femaleStaffOnrollToday),
      'femaleStaffJoinedToday': data.fold(0, (sum, item) => sum + item.femaleStaffJoinedToday),
      'femaleStaffLeftToday': data.fold(0, (sum, item) => sum + item.femaleStaffLeftToday),
      'femaleStaffTotalAttr': calculateAverage(data.map((item) => item.femaleStaffTotalAttr).toList()),
      'staffOnrollToday': data.fold(0, (sum, item) => sum + item.staffOnrollToday),
      'staffJoinedToday': data.fold(0, (sum, item) => sum + item.staffJoinedToday),
      'staffLeftToday': data.fold(0, (sum, item) => sum + item.staffLeftToday),
      'staffTotalAttr': calculateAverage(data.map((item) => item.staffTotalAttr).toList()),
    };
  }
}

// Main widget
class AttManpowerStaffTable extends StatefulWidget {
  final String from;
  final String to;
  final String units;

  const AttManpowerStaffTable({
    super.key,
    required this.from,
    required this.to,
    required this.units,
  ...
  @override
  State<AttManpowerStaffTable> createState() => _AttManpowerStaffTableState();
}

class _AttManpowerStaffTableState extends State<AttManpowerStaffTable> {
  late Future<List<AttManpowerData>> attManpower;

  @override
  void initState() {
    super.initState();
    attManpower = fetchAttManpower(widget.from, widget.to, widget.units);
  }
