...
Future<List<FinishDetail>> fetchFinishingData(String from, String to, String unit, String vgUnit) async {
  final response = await http.get(
    Uri.parse('${TBaseURL.baseUrl}operation_wise_vg?type=FINISHING&unit=$unit&from=$from&to=$to&vgUnit=$vgUnit'),
    // Uri.parse('http://172.16.10.11:8000/opration?type=FINISHING&unit=$unit&from=$from&to=$to'),
  );

  if (kDebugMode) {
    print('${TBaseURL.baseUrl}operation_wise_vg?type=FINISHING&unit=$unit&from=$from&to=$to&vgUnit=$vgUnit');
  }
                        ...
                        children: [
                          SingleChildScrollView(
                            ...
                            child: IntrinsicWidth(
                              child: Table(
                                border: TableBorder.all(color: Theme.of(context).colorScheme.secondary.withOpacity(0.6)),
                                    ...
                                    TableRow(
                                      ...
                                      children: [
                                        ...
                                        TableCell(child: Padding(
                                          ...
                                          child: Text(item.qty.toString(),textAlign: TextAlign.end,style: TextStyle(
                                              color: item.isHoliday|| item.isWeekOff ? Colors.black : Theme.of(context).colorScheme.secondary
                                          ),),
                                        )),
                                        TableCell(child: Padding(
                                          padding: const EdgeInsets.only(right: 4,left: 4),
                                          child: Text(item.revenue.toString(),textAlign: TextAlign.end,style: TextStyle(
                                              color: item.isHoliday|| item.isWeekOff ? Colors.black : Theme.of(context).colorScheme.secondary
                                          ),),
                                        )),
                                        TableCell(child: Padding(
                                          padding: const EdgeInsets.only(right: 4,left: 4),
                                          child: Text(item.manPower.toString(),textAlign: TextAlign.end,style: TextStyle(
                                              color: item.isHoliday|| item.isWeekOff ? Colors.black : Theme.of(context).colorScheme.secondary
                                          ),),
                                        )),
                                        TableCell(child: Padding(
                                          padding: const EdgeInsets.only(right: 4,left: 4),
                                          child: Text(item.oThrs.toString(),textAlign: TextAlign.end,style: TextStyle(
                                              color: item.isHoliday|| item.isWeekOff ? Colors.black : Theme.of(context).colorScheme.secondary
                                          ),),
                                        )),
                                        TableCell(child: Padding(
                                          padding: const EdgeInsets.only(right: 4,left: 4),
                                          child: Text(item.oTAmt.toString(),textAlign: TextAlign.end,style: TextStyle(
                                              color: item.isHoliday|| item.isWeekOff ? Colors.black : Theme.of(context).colorScheme.secondary
                                          ),),
                                        )),
                                        TableCell(child: Padding(
                                          padding: const EdgeInsets.only(right: 4,left: 4),
                                          child: Text(item.cost.toString(),textAlign: TextAlign.end,style: TextStyle(
                                              color: item.isHoliday|| item.isWeekOff ? Colors.black : Theme.of(context).colorScheme.secondary
                                          ),),
                                        )),
                                        TableCell(child: Padding(
...
class _FinishStylePageState extends State<FinishStylePage> with TickerProviderStateMixin{
  ...
  void _updateText() {
    ...
    const shortText = 'Finishing Profit & Loss';
  ...
  Future<void> _startTypingSequence() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _isReversing = false;
        _currentMaxLength = 'Finishing Profit & Loss'.length;
      });
    }
  ...
  void _fetchData() {
    ...
    if (_selectedUnit != null) {
      setState(() {
        _cuttingDetailsFuture = fetchFinishingData(from, to, selectedUnitCode,vgUnit);
      });
    }
  }
                                  ...
                                  child: SizedBox(
                                    ...
                                    child: AnimatedContainer(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        border: Border.all(
                                          color: _showRGBBorder ? _borderColor : Theme.of(context).colorScheme.secondary.withOpacity(0.5),
