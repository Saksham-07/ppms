...
Future<List<CuttingDetail>> fetchCuttingData(
    String from, String to, String unit, String vgUnit) async {
  final response = await http.get(
    Uri.parse(
        '${TBaseURL.baseUrl}operation_wise_vg?type=CUTTING&unit=$unit&from=$from&to=$to&vgUnit=$vgUnit'),
...
void navigateToCutting(
    BuildContext context, String fromDate, String toDate, String unit) {
  Navigator.push(
    context,
    MaterialPageRoute(
        builder: (context) =>
            CuttingStylePage(fromDate: fromDate, toDate: toDate, unit: unit)),
  );
}
...
class CuttingTable extends StatelessWidget {
  ...
  CuttingTable({super.key, required this.data, required this.unit});
...
CuttingModel createModel(
    BuildContext context, CuttingModel Function() modelBuilder) {
  return modelBuilder();
}
    ...
    if (_selectedUnit != null) {
      setState(() {
        _cuttingDetailsFuture =
            fetchCuttingData(from, to, selectedUnitCode, vgUnit);
      });
    }
  }
