...
Future<List<FinishingDetail>> fetchFinishingData(String to, String unit) async {
  DateTime now = DateTime.now();
  String from = DateTime(now.year, now.month, 1).toIso8601String().split('T')[0];

  final response = await http.get(
    Uri.parse('${TBaseURL.baseUrl}stylewise?type=FINISHING&unit=$unit&from=$from&to=$to'),
  );
  print('${TBaseURL.baseUrl}stylewise?type=FINISHING&unit=$unit&from=$from&to=$to');
...
class _FinishingStyleDropPageState extends State<FinishingStyleDropPage> {
  ...
  void _fetchData() {
    ...
    if (_selectedUnit != null) {
      setState(() {
        _cuttingDetailsFuture = fetchFinishingData(to, selectedUnitCode);
      });
    }
  }
